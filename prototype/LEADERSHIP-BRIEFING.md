# GSQ DD&T Leadership Briefing (working copy)

The live workbench is **GSQ EP Prototype**. This copy is the Joel / site-LT briefing experience. Do not mix the two. Do not edit this copy unless asked.

## Two apps

| App | Role | Where |
| --- | --- | --- |
| **GSQ EP Prototype** (original) | **Live workbench** | App `6578df9f-308b-466e-ac80-3f99f06830c8` — see [`ORIGINAL-APP-FROZEN.md`](ORIGINAL-APP-FROZEN.md) |
...
Vibe has no Save as. The copy is a **new** project created from the briefing spec.

## How to iterate the copy

1. Open only the briefing Vibe project `d7c5eb38-…`.
...
Ask Copilot free-text calls the published **standard-harness** Copilot Studio agent **GSQ DD and T Executive Platform Agent** (schema `new_gsqDdtExecPlatformStd`) via `ExecuteCopilotAsyncV2`. First open: Allow the Microsoft Copilot Studio connection if prompted. Studio: https://copilotstudio.microsoft.com/environments/0f7bbacc-ed0b-efbc-9497-b5b75c96f91e/bots/145ef9c6-06af-f111-aaac-7c1e528d5edf/overview

The same agent and connector are live on **GSQ EP Prototype**. The older GitHub Copilot harness agent **GSQDDTExecPlatformAgent** (`cr8d8_gsqddtexecplatformagent_350mvj`) cannot be used by this connector.
...
Both published apps can carry two extra connection references besides MCP: Microsoft Copilot Studio (`1c5f2396-6f9c-43e5-bc7e-e35e589a5aa2`) and, on the Prototype, SharePoint. First play: Allow **GSQDDTExecPlatformAgent**. A later Vibe Publish updates can strip MCS unless that reference is put back.
